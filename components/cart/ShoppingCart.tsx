...
export default function ShoppingCart() {
    ...
    return (
        <>
            {contents.length ? (
                <>
                    <h2 className=" text-4xl font-bold text-gray-800">Resumen de venta</h2>
                    <ul role="list" className=" mt-6 divide-y divide-gray-200 text-sm font-medium text-gray-500">
                        {contents.map(item => (
                        <ShoppingCartItem 
                                key={item.productId}
                                item={item}
